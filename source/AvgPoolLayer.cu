...
AvgPoolLayer::AvgPoolLayer(int ker_size_p, int stride_p, int pad_p):
    _stride(stride_p),
    _pad(pad_p),
    H(ker_size_p),
    W(ker_size_p)
...
void AvgPoolLayer::set_input(std::shared_ptr<Tensor<float>> input)
{
...
std::shared_ptr<Tensor<float>> AvgPoolLayer::get_output()
{
    return _res;
}
